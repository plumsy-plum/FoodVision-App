  ...
  Widget _buildNutritionFactsTitle(bool isDarkMode) {
    return Text(
      ...
      style: TextStyle(
        fontSize: 22,
        fontWeight: FontWeight.bold,
        color: isDarkMode ? Colors.blueAccent : Color(0xFF388E3C),
      ),
    );
  }
}
...
class NutritionFactsCard extends StatelessWidget {
  ...
  const NutritionFactsCard({super.key, required this.meal, required this.isDarkMode});
...
class NutritionFactRow extends StatelessWidget {
  ...
  const NutritionFactRow({super.key, required this.label, required this.value, required this.isDarkMode});
